...
BinDirFiles=()
#readarray -t BinDirFiles < <(ls "/media/chimba/HGST_DATA/Chimbalix Linux/Chimbalix-Software-Catalog/Installer-SH/installer-data/bin/")
BinDirFiles=( $(ls "/media/chimba/HGST_DATA/Chimbalix Linux/Chimbalix-Software-Catalog/Installer-SH/installer-data/bin/") )

for filename in "${!BinDirFiles[@]}"; do
	CurrentFile="${BinDirFiles[$filename]}"
	awk -i inplace '{if($0=="BinDirFiles=(") $0=$0"\n\"'"$CurrentFile"'\"";print}' "/media/chimba/HGST_DATA/Chimbalix Linux/Chimbalix-Software-Catalog/Installer-SH/installer-data/yy.sh"
done
